...
Use the Figma MCP server for Figma-driven implementation. For setup and debugging details (OAuth config, verification, Codex-specific setup, write-to-canvas prerequisites), see `references/figma-mcp-config.md`.
...
## Write-to-canvas note
- Official Figma docs now treat `use_figma` as a separate write-to-canvas workflow for the remote MCP server.
- In Codex, the recommended setup is to install Figma's write-focused skills in addition to the base `figma` skill, especially `Figma Use`.
- If those write skills are not installed, read workflows can still work while canvas-write workflows may be less reliable.
